...
const subscriptions = [
  { name: "Netflix", amount: 649, cadence: "/ month", next: "24 Sep", icon: "N", color: "#e85d75" },
  { name: "Spotify", amount: 119, cadence: "/ month", next: "28 Sep", icon: "S", color: "#46b96b" },
  { name: "YouTube Premium", amount: 149, cadence: "/ month", next: "03 Oct", icon: "Y", color: "#ef5c5c" },
  { name: "Amazon Prime", amount: 1499, cadence: "/ year", next: "12 Oct", icon: "A", color: "#4f8cf7" },
];
    ...
    <div className="app-shell">
      <aside className={`sidebar ${mobileNavOpen ? "sidebar-open" : ""}`}>
        ...
        <div className="system-card">
          ...
          <div><div className="micro-label">SYSTEM STATUS</div><strong>OPERATIONAL</strong></div>
        ...
        <div className="sidebar-bottom">
          ...
          <div className="profile-row"><div className="avatar">AK</div><div><strong>Arjun Kapoor</strong><span>PERSONAL PROFILE</span></div><MoreHorizontal size={17} /></div>
        </div>
      </aside>
      ...
      <main className="main-shell">
        <header className="topbar">
          ...
          <div className="topbar-actions"><div className="month-pill"><span className="led led-orange" /> SEP 2026</div><button className="icon-button notification-button" aria-label="Notifications" onClick={() => notify("You have 2 fresh observations") }><Bell size={18} /><i>2</i></button><button className="avatar avatar-small" onClick={() => navigate("settings")} aria-label="Open profile settings">AK</button></div>
        </header>

        <div className="page-content">
          {page === "dashboard" && <DashboardPage income={income} expenses={expenses} balance={balance} savingsRate={savingsRate} categoryTotals={categoryTotals} transactions={transactions} budgets={budgets} onNavigate={navigate} />}
          ...
          {page === "insights" && <InsightsPage expenses={expenses} categoryTotals={categoryTotals} transactions={transactions} onNavigate={navigate} />}
...
function InsightsPage({ expenses, categoryTotals, transactions, onNavigate }: { expenses: number; categoryTotals: { name: string; value: number }[]; transactions: Transaction[]; onNavigate: (page: Page) => void }) {
...
function SettingsPage({ notify }: { notify: (message: string) => void }) {
  ...
  return <div className="page-stack page-enter"><PageIntro eyebrow="SYSTEM / PROFILE CONFIGURATION" title="Tune your control console." description="Manage how FinPilot presents your personal financial signal." actions={<span className="settings-id">PROFILE / AK-01</span>} /><div className="settings-grid"><section className="panel settings-profile"><div className="large-avatar">AK</div><div><div className="micro-label">PRIMARY PROFILE</div><h2>Arjun Kapoor</h2><p>Personal finance workspace · India / INR</p></div><button className="mechanical-button secondary-button" onClick={() => notify("Profile editing is ready for your next import") }><Pencil size={15} /> EDIT PROFILE</button></section><section className="panel settings-section"><PanelHeading eyebrow="PREFERENCES" title="Display and alerts" action={<Settings2 size={18} />} /><SettingToggle label="Demo mode" description="Keep the realistic local dataset available for walkthroughs." checked={demo} onChange={() => setDemo(!demo)} /><SettingToggle label="Pattern notifications" description="Show a signal when spending differs from your typical rhythm." checked={notifications} onChange={() => setNotifications(!notifications)} /><SettingToggle label="Compact ledger view" description="Use denser rows in the transaction table." checked={false} onChange={() => notify("Compact view preference noted")} /></section><section className="panel settings-section"><PanelHeading eyebrow="DATA & SAFETY" title="Local workspace" action={<ShieldCheck size={18} />} /><div className="settings-row"><div><strong>Connected data</strong><span>28 transactions · 3 months indexed</span></div><span className="complete-chip"><CheckCircle2 size={14} /> READY</span></div><div className="settings-row"><div><strong>Currency</strong><span>Indian Rupee (₹) · en-IN formatting</span></div><span className="mono">INR</span></div><div className="settings-row"><div><strong>Processing mode</strong><span>Local deterministic demo logic</span></div><span className="mono">OFFLINE</span></div></section></div></div>;
}
