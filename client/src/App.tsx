    ...
    <div className="app-shell">
      <aside className={`sidebar ${mobileNavOpen ? "sidebar-open" : ""}`}>
        ...
        <div className="system-card">
          ...
          <div><div className="micro-label">SYSTEM STATUS</div><strong>{isAuthenticated ? "MANUS CONNECTED" : "DEMO MODE"}</strong></div>
        ...
        <div className="sidebar-bottom">
          ...
          <div className="profile-row"><div className="avatar">{initials(displayName)}</div><div><strong>{displayName}</strong><span>{isAuthenticated ? "MANUS ACCOUNT" : "LOCAL DEMO PROFILE"}</span></div><MoreHorizontal size={17} /></div>
        </div>
      </aside>
...
function SettingsPage({ notify, user, isAuthenticated, onLogin, onLogout }: { notify: (message: string) => void; user: { name: string | null; email: string | null; openId: string } | null; isAuthenticated: boolean; onLogin: () => void; onLogout: () => void }) {
  ...
  const displayName = user?.name || "Demo operator";
  return <div className="page-stack page-enter"><PageIntro eyebrow="SYSTEM / PROFILE CONFIGURATION" title="Tune your control console." description="Manage how FinPilot presents your personal financial signal." actions={<span className="settings-id">{isAuthenticated ? "MANUS OAUTH / ACTIVE" : "LOCAL DEMO / GUEST"}</span>} /><div className="settings-grid"><section className="panel settings-profile"><div className="large-avatar">{initials(displayName)}</div><div><div className="micro-label">PRIMARY PROFILE</div><h2>{displayName}</h2><p>{isAuthenticated ? user?.email || "Authenticated Manus account" : "Local demo workspace · India / INR"}</p></div>{isAuthenticated ? <button className="mechanical-button secondary-button" onClick={onLogout}><LogOutIcon /> SIGN OUT</button> : <button className="mechanical-button primary-button" onClick={onLogin}><ShieldCheck size={15} /> SIGN IN WITH MANUS</button>}</section><section className="panel settings-section"><PanelHeading eyebrow="PREFERENCES" title="Display and alerts" action={<Settings2 size={18} />} /><SettingToggle label="Demo mode" description="Keep the realistic local dataset available for walkthroughs." checked={demo} onChange={() => setDemo(!demo)} /><SettingToggle label="Pattern notifications" description="Show a signal when spending differs from your typical rhythm." checked={notifications} onChange={() => setNotifications(!notifications)} /><SettingToggle label="Compact ledger view" description="Use denser rows in the transaction table." checked={false} onChange={() => notify("Compact view preference noted")} /></section><section className="panel settings-section"><PanelHeading eyebrow="DATA & SAFETY" title="Workspace connection" action={<ShieldCheck size={18} />} /><div className="settings-row"><div><strong>Identity connection</strong><span>{isAuthenticated ? `Manus OAuth session · ${user?.openId.slice(0, 10)}…` : "No Manus session connected"}</span></div><span className={isAuthenticated ? "complete-chip" : "settings-id"}>{isAuthenticated ? <><CheckCircle2 size={14} /> CONNECTED</> : "GUEST"}</span></div><div className="settings-row"><div><strong>Transaction source</strong><span>Demo dataset · bank aggregation requires a separate provider</span></div><span className="mono">DEMO</span></div><div className="settings-row"><div><strong>Currency</strong><span>Indian Rupee (₹) · en-IN formatting</span></div><span className="mono">INR</span></div></section></div></div>;
}
